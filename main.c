...
///////////////////////////
const char blankmap_tile[1] = {0x00};
...
//    result = collisionmap[tileindexTL] == 1; 
...
void ohjoy(){
        ...
        if(!(joypad()& J_UP)){
            if(if_can_move(player.x, player.y +1)){
            player.y += 1;
            movegamecharacter(&player, player.x, player.y);
        }
            if(player.y >= 130){
